...
void application_init ( void ) 
{
    ...
    wifi_cfg_t cfg1;
    airquality8_cfg_t cfg2;  /**< Click config object. */
    ...
    len_out = pack_device_id_data("TNB000011", id_buf);
    wifi_generic_write(&wifi, id_buf, len_out);
    
    Delay_ms(2000);
    
    airquality8_cfg_setup( &cfg2 );
    AIRQUALITY8_MAP_MIKROBUS( cfg2, MIKROBUS_2 );
    if ( I2C_MASTER_ERROR == airquality8_init( &airquality8, &cfg2 ) ) 
    {
...
{   
    ...
    len_out = pack_sensors_data((uint8_t *)&air_data, sizeof(air_data), buf, 1);
